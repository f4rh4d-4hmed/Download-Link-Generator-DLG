...
class TorrentSearchWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Torrent Magnet Search")
    ...
    def search(self):
        ...
        self.show_results(search_input)
    ...
    def show_results(self, search_input):
        # Clear existing layout
        for i in reversed(range(self.scroll_layout.count())):
            self.scroll_layout.itemAt(i).widget().setParent(None)

        if search_input == "":
            self.show_debug("Please enter a search query.", error=True)
        else:
            results_found = False
            for name, magnet_link in self.torrent_data.items():
                if search_input in name:
                    self.add_result(name, magnet_link)
                    results_found = True

            if not results_found:
                self.show_debug(f"No results found for '{search_input}'. Scraping now...")
            ...
            if response.status_code == 200:
                ...
                for link in links:
                    ...
                    if magnet_link:
                        ...
                        self.add_result(directory_name, magnet_link)  # Add result to search results
